...
function verificarSesionAdmin() {
    ...
    if (!adminLoggedIn) {
        ...
        window.location.href = '/scr/tienda/login.html';
        return false;
    }

    // Mostrar información del admin logueado
    const adminEmail = localStorage.getItem('adminEmail');
    console.log('Admin logueado:', adminEmail);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Agregar botón de cerrar sesión al nav si no existe
    const nav = document.querySelector('nav ul');
    if (nav && !document.querySelector('.logout-btn')) {
        const logoutItem = document.createElement('li');
        logoutItem.innerHTML = '<a href="#" onclick="cerrarSesionAdmin()" class="logout-btn">🚪 Cerrar Sesión</a>';
